...
fun MainScreen() {
    ...
    // [ARCHITECTURAL REFINEMENT] O BackHandler agora gerencia a navegação de retorno em
    // diferentes níveis de estado. Se uma receita estiver sendo exibida, ele lida com o
    // retorno para a lista. Caso contrário, ele trata da navegação para a HomeScreen a
    // partir de outras seções principais. O parâmetro `enabled` garante que o handler
    // só esteja ativo quando houver uma ação de "voltar" a ser executada.
    BackHandler(enabled = recipeId != null || currentScreen !is Screen.Home) {
        if (recipeId != null) {
